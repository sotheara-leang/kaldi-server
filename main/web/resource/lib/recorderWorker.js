...
this.onmessage = function(e){
  switch(e.data.command){
    ...
    case 'exportWAV':
      ...
      break;
    case 'export16kMonoWav':
      export16kMonoWav(e.data.type, e.data.buffer);
      break;
    case 'getBuffer':
      getBuffer();
      break;
...
function export16kMonoWav(type, interleavedBuffer) {
  monoBuffer = stereoToMono(interleavedBuffer);
  var dataview = encode16kWAV(monoBuffer);
  var audioBlob = new Blob([dataview], { type: type });
...
function encode16kWAV(samples){
  var buffer = new ArrayBuffer(44 + samples.length * 2);
  var view = new DataView(buffer);
  sampleRate = 16000; // This is what kaldi needs; we have already downsampled to 16k
  /* RIFF identifier */
  writeString(view, 0, 'RIFF');
  /* file length */
  view.setUint32(4, 32 + samples.length * 2, true);
